...
async function upsertTableTenant(open_id: string) {
  //todo: upsert to update
  //todo: replace list to get with index
  const { data: tenants } = await client.models.Tenant.list({
    filter: { douyin_openid: { eq: open_id } }
  });
  if (tenants.length > 0) {
    return tenants[0].id;
  } else {
    const { errors, data: newTenant } = await client.models.Tenant.create({
...
export default function authDouyin() {
  ...
  async function authDouyin() {
    ...
    //3. upsert tables
    let tenant_id = await upsertTableTenant(open_id);
